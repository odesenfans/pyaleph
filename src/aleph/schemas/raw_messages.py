...
class BaseRawMessage(BaseModel):
    """
...
class RawAggregateMessage(BaseRawMessage):
    type: Literal[MessageType.aggregate]  # type: ignore
    content: Optional[AggregateContent] = None


class RawForgetMessage(BaseRawMessage):
    type: Literal[MessageType.forget]  # type: ignore
    content: Optional[ForgetContent] = None


class RawPostMessage(BaseRawMessage):
    type: Literal[MessageType.post]  # type: ignore
    content: Optional[PostContent] = None


class RawProgramMessage(BaseRawMessage):
    type: Literal[MessageType.program]  # type: ignore
    content: Optional[ProgramContent] = None


class RawStoreMessage(BaseRawMessage):
    type: Literal[MessageType.store]  # type: ignore
    content: Optional[StoreContent] = None


MESSAGE_TYPE_TO_CLASS = {
    MessageType.aggregate: RawAggregateMessage,
    MessageType.forget: RawForgetMessage,
    MessageType.post: RawPostMessage,
    MessageType.program: RawProgramMessage,
    MessageType.store: RawStoreMessage,
}


def parse_message(message_dict: Any) -> BaseRawMessage:
