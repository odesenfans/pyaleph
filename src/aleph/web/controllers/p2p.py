...
def _get_request_status(failed_publications: Collection[Protocol]) -> str:
    status = {
        0: "success",
        1: "warning",
        2: "error",
...
async def pub_json(request: web.Request):
    ...
    status = _get_request_status(failed_publications)
...
async def pub_message(request: web.Request):
    ...
    message_accepted_response = web.json_response(
        {"status": MessageStatus.PENDING}, status=202
    )
    ...
    if processing_status == MessageProcessingStatus.FAILED_REJECTED:
        raise web.HTTPUnprocessableEntity(
            body=f"Failed to process {pending_message.item_hash}"
        )
    if processing_status == MessageProcessingStatus.FAILED_WILL_RETRY:
        return message_accepted_response

    return web.json_response(
        {"status": processing_status.to_message_status().value}, status=200
    )
