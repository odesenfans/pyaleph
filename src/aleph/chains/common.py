...
class MessageProcessor:
    ...
    async def incoming(
        ...
        if existing:
            ...
            try:
                content = await self.storage_manager.get_message_content(
                    pending_message
                )
...
class ChainDataProcessor:
    def __init__(self, storage_manager: StorageManager):
        self.storage_manager = storage_manager

    async def get_chaindata(self, messages, bulk_threshold=2000):
        ...
        if len(content) > bulk_threshold:
            ipfs_id = await self.storage_manager.add_json(chaindata)
            return json.dumps(
    ...
    async def get_chaindata_messages(
        self, chaindata: Dict, context: TxContext, seen_ids: Optional[List] = None
    ):
        config = get_config()
        ...
        if protocol == "aleph-offchain" and version == 1:
            ...
            try:
                content = await self.storage_manager.get_json(
            ...
            if config.ipfs.enabled.value:
                try:
                    ...
                    await asyncio.wait_for(
                        self.storage_manager.pin_hash(chaindata["content"]), timeout=120
                    )
