...
async def test_hash_content_from_network(mocker, use_ipfs: bool):
    ...
    mocker.patch.object(StorageManager, "_verify_content_hash")

    # No files in the local storage
    storage_manager = StorageManager(MockStorageEngine(files={}))
...
async def test_hash_content_from_network_store_value(mocker, use_ipfs: bool):
    ...
    mocker.patch("aleph.storage.p2p_http_request_hash", return_value=expected_content)
    mocker.patch.object(
        StorageManager, "_verify_content_hash", return_value=content_hash
    )
...
async def test_get_valid_json():
    ...
    storage_manager = StorageManager(MockStorageEngine(files={content_hash: json_bytes}))
...
async def test_get_invalid_json():
    ...
    storage_manager = StorageManager(MockStorageEngine(files={"1234": non_json_content}))
...
async def test_get_stored_message_content():
    ...
    message = parse_message(message_dict)
    storage_manager = StorageManager(MockStorageEngine(files={message.item_hash: json_bytes}))
