...
from aleph.schemas.raw_messages import (
    BaseRawMessage,
    RawAggregateMessage,
    RawForgetMessage,
    RawPostMessage,
...
def check_basic_message_fields(raw_message: BaseRawMessage, message_dict: Dict):
    assert raw_message.chain == message_dict["chain"]
    assert raw_message.item_hash == message_dict["item_hash"]
    assert raw_message.sender == message_dict["sender"]
    assert raw_message.type == message_dict["type"]
    assert raw_message.channel == message_dict["channel"]
    assert raw_message.signature == message_dict["signature"]
    assert raw_message.channel == message_dict["channel"]
...
def test_parse_aggregate_inline_message():
    ...
    assert isinstance(message, RawAggregateMessage)
...
def test_parse_post_message_storage_content():
    ...
    message = parse_message(message_dict)
    assert isinstance(message, RawPostMessage)
...
def test_parse_store_message_inline_content():
    ...
    message = parse_message(message_dict)
    assert isinstance(message, RawStoreMessage)
...
def test_parse_forget_message():
    ...
    message = parse_message(message_dict)
    assert isinstance(message, RawForgetMessage)
...
def test_parse_program_message():
    ...
    message = parse_message(message_dict)
    assert isinstance(message, RawProgramMessage)
